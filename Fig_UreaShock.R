...
experiment_name <- "CC" # change these for different environment-specific scripts
clusterdf_name <- "CC_2"
expdf <- filter(finaldf, experiment == "CC")
folder_name <- "CellCycle"

# removing two blocks of genes deleted in the hybrid Spar allele in the tag-seq data cell cycle experiment
omit_list <- c("YLR078C", "YLR077W", "YLR074C", "YLR072W", "YLR075W", "YLR073C",
               "YNL247W", "YNL244C")
# removing
counts <- counts[!(rownames(counts) %in% omit_list),]
counts_allele <- counts_allele[!(rownames(counts_allele) %in% omit_list),]
