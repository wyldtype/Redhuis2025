...
ncol(counts)
# TODO: I'm such a dingus, I had the normalized counts file called "counts_cpm" before and I just kept
# referencing the un-normalized "counts" variable, so everything was un-normalized downstream and I should re-run it all to see if the networks look different
# 4 organisms excluding HAP4 due to its weirdness
counts_cer <- t(counts[, sample_info$organism == "cer" & sample_info$experiment != "HAP4"])
counts_par <- t(counts[, sample_info$organism == "par" & sample_info$experiment != "HAP4"])
counts_hyc <- t(counts[, sample_info$organism == "hyc" & sample_info$experiment != "HAP4"])
counts_hyp <- t(counts[, sample_info$organism == "hyp" & sample_info$experiment != "HAP4"])
...
# and finally, as special control samples, we isolate LowN samples with WT genotypes:
# (these so far haven't been useful, as the point of WGCNA is to look at expression profiles across lots of conditions at once, so we don't want to isolate just WT)
counts_LowN_cer_WT <- t(counts[, sample_info$experiment == "LowN" & sample_info$organism == "cer" & sample_info$genotype == "WT"])
counts_LowN_par_WT <- t(counts[, sample_info$experiment == "LowN" & sample_info$organism == "par" & sample_info$genotype == "WT"])
counts_LowN_hyc_WT <- t(counts[, sample_info$experiment == "LowN" & sample_info$organism == "hyc" & sample_info$genotype == "WT"])
counts_LowN_hyp_WT <- t(counts[, sample_info$experiment == "LowN" & sample_info$organism == "hyp" & sample_info$genotype == "WT"])

# as we will see below, we're going to exclude HAP4 samples from the consensus network 
# due to 1) their weirdness and 2) there not being that many of them anyway.
# However, their gene trees do show good separation (better than any other experiments) 
# so it's worth it to look at them a little more in-depth
...
counts_all12 <- list(CC_cer = counts_CC_cer,
...
geneVar_all12 <- sapply(counts_all12, colVars)
geneVarIsZero_all12 <- geneVar_all12 == 0
dropGenes <- apply(geneVarIsZero_all12, 1, any)
counts_all12 <- lapply(counts_all12, function(x) {
  x <- x[,!dropGenes]
  return(x)
})

# also doing just species versions, still excluding HAP4, dropping all the genes that are dropped in the 12 individual experiments x species
counts_all4 <- list(cer = counts_cer,
                    par = counts_par,
                    hyc = counts_hyc,
                    hyp = counts_hyp)
counts_all4 <- lapply(counts_all4, function(x) {
  x <- x[,!dropGenes]
  return(x)
})
...
rm(counts_LowN_cer, counts_CC_cer, counts_LowPi_cer, 
   ...
   counts_CC_par, counts_LowN_par, counts_LowPi_par) # this order is chaos
...
consensusTOM <- list(cer = pmax(TOM_all12$CC_cer, TOM_all12$LowN_cer, TOM_all12$LowPi_cer),
                     par = pmax(TOM_all12$CC_par, TOM_all12$LowN_par, TOM_all12$LowPi_par),
                     hyc = pmax(TOM_all12$CC_hyc, TOM_all12$LowN_hyc, TOM_all12$LowPi_hyc),
                     hyp = pmax(TOM_all12$CC_hyp, TOM_all12$LowN_hyp, TOM_all12$LowPi_hyp)) # pmin is "parallel" min, takes as many vectors as you give it of the same length and gives the min at each position
...
plotDendroAndColors(dendro = geneTree_all12[[idx_12]], colors = cbind(dynamicColors_all12[[idx_12]], hybridColors_all12[[idx_12]], hybridColors_all12_ds1[[idx_12]], hybridColors_all12_ds0[[idx_12]]),
                    ...
                    dendroLabels = FALSE)
# hybrid is a bit less fuzzy than dynamic, but the big thing is deepSplit=2 looks best
dev.off()
par(mfcol = c(3,4))
for (i in 1:12) {
...
# selecting hybrid deepSplit = 2 as our module definitions
colors_all12 <- hybridColors_all12
consensusColors <- consensusHybridColors
...
for (i in 1:4) {
  ...
  plotDendroAndColors(geneTree_all4[[i]], cbind(colors_all4[[i]], colors_all12[[idx_12+1]], colors_all12[[idx_12+2]], colors_all12[[idx_12+3]]),
                      ...
                      dendroLabels = FALSE) # this function unfortunately does not respect my desire to have a 2x2 grid of plots
} # conglomerate seems fuzzier...---am I sure I used hybrid colors with conglomerate? Yeah pretty sure
...
# consensus (this currently fails because hyc and hyp only have 2 modules so there's no outgroup for the dendrogram)
dev.off()
par(mfrow = c(2,2))
for (i in 1:4) {
...
TOMplotSubset <- function(cts, dissTOM, moduleColors, sampleSize = 1000, plotTitle = "TOM plot") {
  ...
  plotTOM <- selectTOM^10 # I guess 7 is just a power they thought looked pretty
  diag(plotTOM) <- NA
...
TOMplotSubset(cts = counts_all4[[1]], dissTOM = dissTOM_all4[[1]], 
              moduleColors = colors_all4[[1]], plotTitle = paste(names(counts_all4)[1], "conglomerate"))
# par
TOMplotSubset(cts = counts_all4[[2]], dissTOM = dissTOM_all4[[2]], 
              moduleColors = colors_all4[[2]], plotTitle = paste(names(counts_all4)[2], "conglomerate"))
# hyc
TOMplotSubset(cts = counts_all4[[3]], dissTOM = dissTOM_all4[[3]], 
              moduleColors = colors_all4[[3]], plotTitle = paste(names(counts_all4)[3], "conglomerate"))
# hyp
TOMplotSubset(cts = counts_all4[[4]], dissTOM = dissTOM_all4[[4]], 
              moduleColors = colors_all4[[4]], plotTitle = paste(names(counts_all4)[4], "conglomerate"))
# consensus 
# cer
TOMplotSubset(cts = counts_all4[[1]], dissTOM = consensusDissTOM[[1]], 
              moduleColors = consensusColors[[1]], plotTitle = paste(names(counts_all4)[1], "consensus"))
# par
TOMplotSubset(cts = counts_all4[[2]], dissTOM = consensusDissTOM[[2]], 
              moduleColors = consensusColors[[2]], plotTitle = paste(names(counts_all4)[2], "consensus"))
# hyc
TOMplotSubset(cts = counts_all4[[3]], dissTOM = consensusDissTOM[[3]], 
              moduleColors = consensusColors[[3]], plotTitle = paste(names(counts_all4)[3], "consensus"))
# hyp
TOMplotSubset(cts = counts_all4[[4]], dissTOM = consensusDissTOM[[4]], 
              moduleColors = consensusColors[[4]], plotTitle = paste(names(counts_all4)[4], "consensus"))
...
# consensus and conglomerate networks
