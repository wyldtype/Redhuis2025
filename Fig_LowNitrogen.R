...
experiment_name <- "LowN" # change these for different environment-specific scripts
clusterdf_name <- "LowN_2"
expdf <- filter(finaldf, experiment == "LowN")
...
for (g in code_order) {
  ...
  if (grepl("cons", g) | grepl("^dyn", g)) { # center and scaly y axis
    plotlist[[g]] <- annotate_figure(plotGenes(.gene_idxs = gene_idxs, .quartet = TRUE, .plotlims = c(-1.1, 1.1),
                                               .experiment_name = experiment_name, .normalization = "scale"),
                                     top = paste0(length(gene_idxs), " / ", nrow(expdf), " genes"))
  }
  else { # log 2 y axis (there's a level difference)
